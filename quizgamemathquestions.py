...
class MathQuestion(Question):
    ...
    def __init__(self, difficulty):
        ...
        #og is used to gen the operator
        #o is the operator string itself
        #a-d are the operands present
        q = ""
        
        #check the difficulty value
        #based on the value the question will have more operators and bigger numbers
        #easy questions
        if (difficulty == 0):
            ...
            self._rightAnswer = eval(q)
            ans = [int(eval(q)), -int(eval(q)), random.randint(0, 9), random.randint(0, 9)]
            random.shuffle(ans)
            self._answers = ans
        ...
        if (difficulty == 1):
            ...
            self._question = q
            self._rightAnswer = eval(q)
            ans = [int(eval(q)), -int(eval(q)), random.randint(-1000, 1000), random.randint(-1000, 1000)]
            random.shuffle(ans)
            self._answers = ans
...
easyHistoryQuestions = [Question("What year did America gain its independence?", "1776","1861","1492","1607"), Question("When did WWII start?", "1939", "1941", "1945", "1937")]
medHistoryQuestions = [Question("What was the last Chinese dynasty?", "Qing","Ming","Han","Yuan"), Question("When was the Declaration of independence signed?", "August 2, 1776", "July 4, 1776", "May 4, 1776", "September 2, 1776"), Question("What city was the first capital of the United States?", "New York City", "Washington D.C.", "Hampton", "Jersey City")]
hardHistoryQuestions = [Question("What battle led to the dissolution of the Holy Roman Empire?", "The Battle of Austerlitz","The Battle of Waterloo","The Siege of Troy","The Battle of Britain")]

easyMathQuestions = []
for i in range(0, 9):
    easyMathQuestions.append(MathQuestion(0))
medMathQuestions = []
for i in range(0, 9):
    medMathQuestions.append(MathQuestion(1))
hardMathQuestions = []
for i in range(0, 9):
    hardMathQuestions.append(MathQuestion(2))
